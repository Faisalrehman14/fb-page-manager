...
/** Facebook sync often keeps unread_count>0 after a page reply — do not store that in DB. */
    ...
    try {
        ...
        const [rows] = await pool.query(
            `SELECT page_id, COUNT(*) AS cnt FROM messenger_conversations
             WHERE page_id IN (${placeholders}) AND is_unread > 0 AND COALESCE(last_from_me, 0) = 0
...
async function updateConversationFromMessage(message) {
    ...
    try {
        if (lastFromMe !== undefined) {
            const fromPage = lastFromMe ? 1 : 0;
            await pool.query(`
                UPDATE messenger_conversations
                SET snippet = ?, updated_at = ?, last_from_me = ?,
                    is_unread = IF(? = 1, 0, is_unread)
                WHERE id = ?
            `, [snippet, updatedAt, fromPage, fromPage, threadId]);
        } else {
            await pool.query(`
                UPDATE messenger_conversations
        ...
        const [updatedConvsResult, unreadResult] = await Promise.all([
            ...
            conn.query(
                `SELECT COUNT(*) AS total FROM messenger_conversations
                 WHERE page_id = ? AND is_unread > 0 AND COALESCE(last_from_me, 0) = 0 AND COALESCE(can_reply, 1) = 1`,
                [pageId]
            )
        ]);
...
async function createSchedule({ fb_user_id, pages, message, image_url, delay_ms, scheduled_at }) {
    ...
    const [result] = await pool.query(
        ...
        [fb_user_id, firstPage.id || '', firstPage.name || null, firstPage.token || '',
         pagesJson, message, image_url || null, delay_ms || 400, scheduled_at]
    );
...
async function getUserPreferences(fb_user_id) {
    ...
    return {
        ...
        default_delay_ms: Math.max(25, parseInt(r.default_delay_ms, 10) || 400),
...
async function upsertUserPreferences(fb_user_id, patch = {}) {
    ...
    const next = {
        ...
        default_delay_ms: patch.default_delay_ms !== undefined
            ? Math.max(25, parseInt(patch.default_delay_ms, 10) || 400)
