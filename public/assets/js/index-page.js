...
function showUpgradeModal(reason){
  ...
  if(sub)sub.textContent='Pay with card (Stripe) or crypto (Binance Pay).';
...
window.showPaymentPopup=async function(plan){
  ...
  await startCheckoutWithProvider(plan,hasBinance?'binance':'stripe');
};
...
(function () {
  ...
  if (landToggle) {
    landToggle.addEventListener('click', function () {
      toggleAppTheme();
    });
  }
